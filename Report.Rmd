...
The entire analisys is done in R on the probeset data (latter annotated).
The scripts are available at http://github.com/kristiyanto
...
```{r, echo=FALSE, cache=TRUE}
    ...
    hist(fit2$F.p.value, main=paste("All. F. P Value"), freq=T, breaks=seq(0,1,p), xlab="PValue", col=colors.12)
...
The list of the genes that are differentially expressed based on F. P Value (Significance `r p`)
...
## Heatmap
Heat-map for all experiments for the genes above. (`r p=0.01` P= `r p`). Y Axis is the probeset, Z Axis is the experiments.
...
The list of the genes that are differentially expressed for group 1 & 2 (Significance `r p`)
...
Plotting those genes into a heatmap
